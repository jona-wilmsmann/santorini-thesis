...
fn alpha_beta_sorted_minimax_internal<GS: GameState + MinimaxReady>(game_state: &GS, maximizing_player: bool, depth: usize, mut alpha: f32, mut beta: f32, reused_children_vec: &mut Vec<GS>, evaluated_states: &mut usize) -> f32 {
    *evaluated_states += 1;
    ...
    game_state.get_children_states_reuse_vec(reused_children_vec);
    GS::sort_children_states(reused_children_vec, maximizing_player, depth, &mut MinimaxCache::new());

    let mut reusable_vec_for_children = Vec::with_capacity(32);
    if maximizing_player {
        let mut max_evaluation = f32::NEG_INFINITY;
        for child in reused_children_vec {
            let evaluation = alpha_beta_sorted_minimax_internal(child, false, depth - 1, alpha, beta, &mut reusable_vec_for_children, evaluated_states);
            if evaluation > max_evaluation {
                max_evaluation = evaluation;
                if max_evaluation >= beta {
                    break;
        ...
        let mut min_evaluation = f32::INFINITY;
        for child in reused_children_vec {
            let evaluation = alpha_beta_sorted_minimax_internal(child, true, depth - 1, alpha, beta, &mut reusable_vec_for_children, evaluated_states);
            if evaluation < min_evaluation {
                min_evaluation = evaluation;
                if min_evaluation <= alpha {
                    break;
...
pub fn alpha_beta_sorted_minimax<GS: GameState + MinimaxReady>(game_state: &GS, depth: usize) -> (f32, usize) {
    let mut evaluated_states = 0;
    let mut reused_children_vec = Vec::with_capacity(32);
    let result = alpha_beta_sorted_minimax_internal(game_state, game_state.is_player_a_turn(), depth, f32::NEG_INFINITY, f32::INFINITY, &mut reused_children_vec, &mut evaluated_states);
    return (result, evaluated_states);
}
...
pub fn minimax<GS: GameState + MinimaxReady>(game_state: &GS, depth: usize, mut alpha: f32, mut beta: f32, cache: &mut MinimaxCache<GS>) -> f32 {
    ...
    GS::sort_children_states(&mut children_states, game_state.is_player_a_turn(), depth, cache);
