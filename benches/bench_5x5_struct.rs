...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    let random_states: Vec<GameState5x5Struct> = (0..1000000).map(|_| GameState5x5Struct::from_generic_game_state(&GenericSantoriniGameState::<5, 5, 2>::generate_random_state_rng(&mut rng))).collect();
